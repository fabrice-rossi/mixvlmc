...
The VLMC above is obviously overfitting to the time series, as illustrated by the 0/1 transition probabilities. A classical way to select a good model is to minimize the [BIC](https://en.wikipedia.org/wiki/Bayesian_information_criterion). In `mixvlmc` this can be done easily by *pruning* a complex VLMC using a combination of `cutoff()` and `prune()`, as follows (see `vignette("variable-length-markov-chains")` for details):
