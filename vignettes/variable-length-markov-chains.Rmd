...
-   `max_depth`: the largest order/memory considered for the VLMC
    ...
    if the final model has contexts that reach the maximum value;
...
complexity of the VLMC. An important point to note is that a *higher*
value of `alpha` leads to a more complex model as does a *lower* value
of `cutoff`.
...
`cutoff` (or `alpha`) and to use an information criterion to find the
best model. A conservative value of `cutoff` is a small one, while
`alpha` should be high to be conservative. A possible choice is to use
the BIC inspired `cutoff` divided by a fixed value, for instance
$\frac{1}{4}(|S|-1)\log n$.
...
techniques, for instance based on the quality of the predictions of the model on a
hold-out example. For a more standard use, the `tune_vlmc()` provides a fully
automated solution, including the choice of conservative values of the initial cut 
off, as demonstrated below:
