...
past values o of $\mathbf{Y}$ but in practice we allow a dependency to
...
The `draw.covlmc()` function is more advanced that its VLMC counterpart. In particular
it reports in more details p-values associated to pruning operations. 
The "merging" p-value corresponds to replacing the some of the contexts
by a single join model while the "collapsing" p-value is associated to pruning
all the sub-contexts of a context. P-values associated to specific models correspond
to reducing the memory of the corresponding model, that is discarding the dependency 
...
function. COVLMC contexts have additional characteristics compared to VLMC and context trees.
