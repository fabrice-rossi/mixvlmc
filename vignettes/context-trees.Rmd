...
Notice that the number of potential contexts grows exponentialy  with the length of the time series and it is therefore advisable to keep `max_depth` to a reasonable value. Let us consider a simple example.
...
y_ctx_min_1
```
If we rise the depth limit, the number of contexts keep increasing:
```{r}
