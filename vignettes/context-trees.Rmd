...
to operate at the node level, using the `find_sequence()` function. For instance 
exploring the `r context_number(y_ctx_min_1)` contexts obtained above with 
`min_size=1` is not convenient, but we may be interested by e.g. the sequence
`c("a", "a", "a")`. We look for a corresponding node in the tree with
```{r}
