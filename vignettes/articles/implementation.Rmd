...
A context tree is essentially a root list of class `ctx_tree` with at least a 
`vals` component that contains the values allowed in the state space of the 
context tree (accessed via the `states()` function). In general, the list 
contains also two additional components used to speed up basic operations:
...
The root not does not have a `match` attributes has it corresponds to the empty
