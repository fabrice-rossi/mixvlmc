...
test_that("metrics for constant_model work as expected", {
  ...
  cm <- matrix(c(0, 0, 0, length(target)), byrow = TRUE, ncol = 2)
  expect_identical(mm$conf_mat, cm)
  expect_equal(mm$accuracy, sum(diag(cm)) / length(target))
  ...
  cm <- matrix(c(0, 0, table(target2)), byrow = TRUE, ncol = 2)
  expect_identical(mm$conf_mat, cm)
  expect_equal(mm$accuracy, sum(diag(cm)) / length(target))
  ...
  mm <- glm_metrics(model, x, target)
  cm <- matrix(c(nrow(x), rep(0, 8)), byrow = TRUE, ncol = 3)
  expect_identical(mm$conf_mat, cm)
  expect_equal(mm$accuracy, sum(diag(cm)) / length(target))
  expect_equal(mm$auc, NA)
  ...
  mm <- glm_metrics(model, x, target2)
  cm <- matrix(c(table(target2), rep(0, 6)), byrow = TRUE, ncol = 3)
  expect_identical(mm$conf_mat, cm)
  expect_equal(mm$accuracy, sum(diag(cm)) / length(target))
...
test_that("metrics for vglm work as expected", {
  ...
  cm <- table(apply(m_probs, 1, which.max), target)
