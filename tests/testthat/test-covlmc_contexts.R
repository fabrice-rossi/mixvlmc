test_that("context format is consistent", {
  ...
  hsize_ctx <- contexts(model, type = "data.frame", hsize = TRUE)
  ...
  full_ctx_model_hsize <- contexts(model, type = "data.frame", frequency = "detailed", model = "coef", hsize = TRUE)
  expect_named(full_ctx_model_hsize, c("context", "freq", "A", "B", "C", "coef", "hsize"))
  super_full_ctx_model_hsize <- contexts(model, type = "data.frame", frequency = "detailed", positions = TRUE, model = "coef", hsize = TRUE)
  ...
  ctx_model_with_metrics <- contexts(model, type = "data.frame", model = "full", metrics = TRUE)
  expect_named(ctx_model_with_metrics, c("context", "model", "accuracy", "auc"))
  ctx_model_with_merging <- contexts(model, type = "data.frame", model = "full", merging = TRUE)
...
test_that("models are consistent", {
  ...
  for (engine in c("glm", "multinom")) {
    ...
    ctx_m_cov_c <- contexts(m_cov, type = "data.frame", model = "coef", hsize = TRUE)
...
test_that("covariate depth is reported consistently", {
  ...
  for (k in 1:5) {
    ...
    ctx_m_cov <- contexts(model, type = "data.frame", hsize = TRUE)
