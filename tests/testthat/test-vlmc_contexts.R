test_that("context format is consistent", {
  ...
  m_full_ctx_co <- contexts(model, type = "data.frame", frequency = "detailed", cutoff = "native", metrics = TRUE)
  expect_named(m_full_ctx_co, c("context", "freq", "A", "B", "C", "cutoff", "accuracy", "auc"))
  m_super_full_ctx_co <- contexts(model, type = "data.frame", frequency = "detailed", positions = TRUE, cutoff = "native", metrics = TRUE)
